...
| **A5 — Someone deleting evidence to destroy the case** | Has the phone, wants evidence gone | Server-side immutable copy: Firestore/Storage rules deny update and delete to everyone, including the account owner |
...
```
Firestore:  /users/{uid}/evidence/{evidenceId}    ← metadata only
